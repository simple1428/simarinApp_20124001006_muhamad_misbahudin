import { Head, Link, useForm } from '@inertiajs/react';
import type { FormEvent } from 'react';
import {
    Anchor,
    ArrowLeft,
    CheckCircle2,
    Compass,
    Edit3,
    MapPin,
    Navigation,
    Save,
} from 'lucide-react';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import InputError from '@/components/input-error';

interface RouteItem {
    id: number;
    asal: string;
    tujuan: string;
    status: 'aktif' | 'nonaktif';
}

interface Props {
    route: RouteItem;
}

export default function ShippingRouteEdit({ route }: Props) {
    const form = useForm({
        asal: route.asal,
        tujuan: route.tujuan,
    });

    const submit = (e: FormEvent) => {
        e.preventDefault();
        form.put(`/shipping-routes/${route.id}`);
    };

    return (
        <>
            <Head title={`Edit Nama Rute ${route.asal} → ${route.tujuan} - Operator SIMARIN`} />

            <div className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6 lg:p-8">
                {/* HEADER & BACK BUTTON */}
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-500/10 px-3 py-0.5 text-xs font-semibold text-blue-600 dark:text-blue-400">
                                <Edit3 className="size-3.5" />
                                PERBARUI PENAMAAN RUTE
                            </span>
                        </div>
                        <h1 className="mt-1 text-2xl font-black tracking-tight text-foreground sm:text-3xl">
                            Edit: {route.asal} &rarr; {route.tujuan}
                        </h1>
                        <p className="text-xs text-muted-foreground sm:text-sm">
                            Sesuaikan penamaan resmi pelabuhan asal dan pelabuhan tujuan akhir penyeberangan.
                        </p>
                    </div>

                    <Button variant="outline" size="sm" asChild className="h-9 text-xs">
                        <Link href="/shipping-routes">
                            <ArrowLeft className="mr-1.5 size-3.5" />
                            Kembali
                        </Link>
                    </Button>
                </div>

                {/* FORM CARD */}
                <Card className="border-border/80 shadow-xs">
                    <CardHeader className="border-b border-border/60 pb-4">
                        <CardTitle className="flex items-center gap-2 text-base font-bold">
                            <Navigation className="size-4 text-blue-600" />
                            Penamaan Pelabuhan Rute
                        </CardTitle>
                        <CardDescription className="text-xs">
                            Perubahan nama rute akan langsung disinkronkan pada seluruh tampilan log manifes pelayaran.
                        </CardDescription>
                    </CardHeader>

                    <CardContent className="pt-6">
                        <form onSubmit={submit} className="space-y-5">
                            {/* PELABUHAN ASAL */}
                            <div className="space-y-1.5">
                                <Label htmlFor="asal" className="text-xs font-semibold">
                                    Nama Pelabuhan Asal Keberangkatan <span className="text-rose-500">*</span>
                                </Label>
                                <Input
                                    id="asal"
                                    type="text"
                                    required
                                    value={form.data.asal}
                                    onChange={(e) => form.setData('asal', e.target.value)}
                                    className="h-10 text-xs"
                                />
                                <InputError message={form.errors.asal} />
                            </div>

                            {/* PELABUHAN TUJUAN */}
                            <div className="space-y-1.5">
                                <Label htmlFor="tujuan" className="text-xs font-semibold">
                                    Nama Pelabuhan Tujuan Akhir <span className="text-rose-500">*</span>
                                </Label>
                                <Input
                                    id="tujuan"
                                    type="text"
                                    required
                                    value={form.data.tujuan}
                                    onChange={(e) => form.setData('tujuan', e.target.value)}
                                    className="h-10 text-xs"
                                />
                                <InputError message={form.errors.tujuan} />
                            </div>

                            {/* ACTIONS */}
                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-border/60">
                                <Button variant="outline" size="sm" asChild className="h-9 text-xs">
                                    <Link href="/shipping-routes">Batal</Link>
                                </Button>
                                <Button
                                    type="submit"
                                    size="sm"
                                    disabled={form.processing}
                                    className="h-9 cursor-pointer bg-gradient-to-r from-blue-600 to-indigo-600 px-4 text-xs font-bold text-white shadow-md shadow-blue-600/20 hover:from-blue-500 hover:to-indigo-500"
                                >
                                    <Save className="mr-1.5 size-3.5" />
                                    {form.processing ? 'Menyimpan...' : 'Simpan Perubahan'}
                                </Button>
                            </div>
                        </form>
                    </CardContent>
                </Card>
            </div>
        </>
    );
}

ShippingRouteEdit.layout = {
    breadcrumbs: [
        {
            title: 'Master Rute Penyeberangan',
            href: '/shipping-routes',
        },
        {
            title: 'Edit Nama Rute',
            href: '#',
        },
    ],
};
